...
static int do_transcodeFrame(CodecEngine* _ce,
                             ...
                             char* _jpegImageBuffer, int* _jpegImageSize)
{
...
//  tcInArgs.alg.autoDetectHsv = _targetDetectCommand->m_cmd;
  ...
  tcOutBufDesc.bufSizes[0] = _srcFrameSize;//_dstFrameSize;
...
//  Memory_cacheInv(_ce->m_dstBuffer, _ce->m_dstBufferSize); // invalidate *whole* cache, not only expected portion, just in case
  ...
  if (processResult != IVIDTRANSCODE_EOK)
  {
    fprintf(stderr, "VIDTRANSCODE_process(%zu -> %zu) failed: %"PRIi32"/%"PRIi32"\n",
            _srcFrameSize, 0 /* _dstFrameSize */, processResult, tcOutArgs.base.extendedError);
    return EILSEQ;
  }
...
*/
  ...
  if(tcOutArgs.alg.size > JPEG_IMAGE_BUFFER_SIZE) {
    fprintf(stderr, "do_transcode_frame: too small jpg buffer! image size: %d\n", tcOutArgs.alg.size);

    return 0;
  }

  *_jpegImageSize = tcOutArgs.alg.size;
  #warning This memcpy is blocking high fps
  if(_ce->m_videoOutEnable) {
    memcpy(_jpegImageBuffer, _ce->m_dstBuffer, *_jpegImageSize); 
  }
                              ...
                              char* _jpegImageBuffer, int* _jpegImageSize)
{
  ...
  res = do_transcodeFrame(_ce,
                          ...
                          _targetLocation,
                          _targetDetectParamsResult, */_jpegImageBuffer, _jpegImageSize);
